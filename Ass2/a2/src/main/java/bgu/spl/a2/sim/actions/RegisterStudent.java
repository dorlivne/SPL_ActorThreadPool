    ...
    protected void start() {
        ...
        for (CoursePrivateState pref : Preferences) {//add all preferences to the list of callbacks
            if (!this.getResult().isResolved()) {
                Action<Boolean> RegStudentConfirmation = new RegStudentConfirmation(this.studentID,this.Preferences.get(index).toString() ,this.Preferences.get(index).getPrequisites(), this.Grades.get(index), this.privateState.getGrades());
                actions.add(RegStudentConfirmation);
                sendMessage(RegStudentConfirmation, this.Preferences.get(index).toString(), new StudentPrivateState());
                then(actions, () -> {
