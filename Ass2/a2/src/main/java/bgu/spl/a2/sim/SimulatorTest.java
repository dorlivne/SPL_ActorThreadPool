...
//    public Timeout globalTimeout = Timeout.seconds(10); // 10 seconds max per method tested
    ...
    @Test
    public void main() {
        Simulator.main(new String[]{"test.json"});
    ...
    private void testCourse(String actor, CoursePrivateState state) {
        switch (actor) {
            ...
            case "TestCourse" : {
                assertTrue("Course " + actor + ": should have 3 Participate In Course action and one unregister " , state.getLogger().containsAll(new ArrayList<>(Arrays.asList("Participate In Course","Unregister"))));
                assertTrue("Course " + actor + ": availableSpots should be 0", state.getAvailableSpots() == 0);
                assertTrue("Course " + actor + ": should have 2 registered students", state.getRegistered() == 2);
                break;
            }
            case "TestCourse2":{
                assertTrue("Course " + actor + ": should have 2 Participate In Course action ", state.getLogger().containsAll(new ArrayList<>(Arrays.asList("Participate In Course"))));
                assertTrue("Course " + actor + ": availableSpots should be 0 or 1", state.getAvailableSpots() == 0 || state.getAvailableSpots() == 1);
                assertTrue("Course " + actor + ": should have 1 registered students or 2", state.getRegistered() == 2 || state.getRegistered() == 1);
            break;
            }
            case "TestCourse3":{
              //  assertTrue("Course " + actor + ": shold have 1 Participate In Course and Register With preferences", state.getLogger().containsAll(new ArrayList<>(Arrays.asList("Participate In Course","Register with Preferences","Add Spaces"))));
                assertTrue("Course " + actor + ": availableSpots should be 100", state.getAvailableSpots() == 100);
                assertTrue("Course " + actor + ": should have 1 reg studen", state.getRegistered() == 1);
            }
        }
    }
    ...
    private void testStudent(String actor, StudentPrivateState state) {
        ...
        switch (actor) {
            ...
            case "Dor":{
                assertTrue("student " + actor + ": signature should be 4424323 or 555353", state.getSignature() == 5555353 || state.getSignature() == 4424232);
                break;}
            case "Maor":{
                assertTrue("Student " + actor + ": no Register With Pref found", state.getLogger().contains(new ArrayList<>(Arrays.asList("Register With Preference"))));
                assertTrue("student " + actor + ": signature should be  555353", state.getSignature() == 5555353 );
                break;}
            case "Amir":{
                assertTrue("student " + actor + ": signature should be 4424323 ",  state.getSignature() == 4424232);
                break;}
    ...
    private void testTest(String actor,PrivateState state){
        assertTrue("Department" + actor + ": should have 3 Add student action in logger", Collections.frequency(state.getLogger(), "Add Student") == 3);
        assertTrue("Department" + actor + ": should have 2 Administrative Check in logger", Collections.frequency(state.getLogger(), "Administrative Check") == 2);
        assertTrue("Department" + actor + ": courselist should be 3",
                ((DepartmentPrivateState) state).getCourseList().containsAll(new ArrayList<>(Arrays.asList("TestCourse","TestCourse2","TestCourse3"))));
        assertTrue("Department" + actor + ": not all students appear in studentslist " +
                        " should have: Maor,Dor,Amir",
                ((DepartmentPrivateState) state).getStudentList().containsAll(new ArrayList<String>(Arrays.asList("Dor","Maor","Amir"))));
    }
